...
// template <> struct select_npy_type<int64_t> { const static NPY_TYPES type =
// NPY_INT64; };
                  ...
                  const long fig_number = 0)
{
       ...
       ++it)
  {
    ...
    {
      PyDict_SetItemString(
          kwargs, it->first.c_str(), PyFloat_FromDouble(std::stod(it->second)));
    }
    else
    {
      PyDict_SetItemString(
...
inline void subplot2grid(long nrows,
                         long ncols,
                         long rowid   = 0,
                         long colid   = 0,
                         long rowspan = 1,
...
template <> struct plot_impl<std::false_type>
{
  ...
  bool
  operator()(const IterableX &x, const IterableY &y, const std::string &format)
  {
    detail::_interpreter::get();
